...
module.exports= (function(name) {
    ...
    function getStackTrace(str) {
        ...
        str= obj.stack+ str;
        if("trace"=== this.type){
            accessLog.trace.call(accessLog, str);
        }else if("debug"=== this.type){
            accessLog.debug.call(accessLog, str);
        }else if("info"=== this.type){
            accessLog.info.call(accessLog, str);
        }else if("warn"=== this.type){
            exceptionLog.warn.call(exceptionLog, str);
        }else if("error"=== this.type){
            //Cat.logEvent("MDBUSS","logger-err", Cat.FAIL, obj.stack+ "\n"+ str );
            exceptionLog.error.call(exceptionLog, str);
        }else if("fatal"=== this.type){
            exceptionLog.fatal.call(exceptionLog, str);
        }else if("proxy"=== this.type){
            proxyLog.info.call(proxyLog, str);
        }
    }
